...
class ContainerMoonPlayer < Container
  ...
  def initialize(x, y)
    ...
    @play.on(:mouse_focus_no_mod) do |widget|
      Moon::Music.playing? ? pause_music : play_music(true)
    end

    @play.on(:mouse_focus_control) do |widget|
      puts "With CONTROL"
      play_music(false)
    end

    @play.on(:mouse_focus_alt) do |widget|
      puts "With ALT"
    end

    @play.on(:mouse_focus_shift) do |widget|
      puts "With SHIFT"
      pause_music
    end

    @play.on(:mouse_focus_super) do |widget|
      puts "With SUPER"
    end

    #@stop.on(:mouse_over) {}

    @stop.on(:mouse_focus) do |widget|
      stop_music
    end
  ...
  def init_spriteset
    @spritesheet_16x16  = Moon::Spritesheet.new("resources/media_buttons_16x16.png", 16, 16)
    @spritesheet_32x32  = Moon::Spritesheet.new("resources/media_buttons_32x32.png", 32, 32)
    @spritesheet_128x16 = Moon::Spritesheet.new("resources/media_buttons_128x16.png", 128, 16)
  end
  ...
  def render
    @spritesheet_32x32.render(@play.x, @play.y, 0, Moon::Music.playing? ? 5 : 1)
    @spritesheet_32x32.render(@stop.x, @stop.y, 0, Moon::Music.stopped? ? 4 : 0)
    @spritesheet_128x16.render(@title.x, @title.y, 0, 10)
    @spritesheet_128x16.render(@seekbar.x, @seekbar.y, 0, Moon::Music.finished? ? 9 : 8)
    @spritesheet_16x16.render(@seek.x, @seek.y, 0, Moon::Music.playing? ? 10 : 11)
...
class State::MoonPlayer < State
  ...
  def init
    ...
    @button_hud = State::KeyHoldTest::Hud_KeyHold.new(device, button, 8, 8)
    @button_hud.x = 0

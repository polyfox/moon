...
MOON_C_API const struct mrb_data_type font_data_type = { "Moon::Font",
                                                         font_free };
...
font_calc_bounds(mrb_state* mrb, mrb_value self)
{
  ...
  mrb_value argv[2] = { mrb_fixnum_value(bounds.x),
                        mrb_fixnum_value(bounds.y) };
...
mmrb_font_init(mrb_state* mrb)
{
  ...
  struct RClass* font_class =
    mrb_define_class_under(mrb, mod, "Font", mrb->object_class);
  MRB_SET_INSTANCE_TT(font_class, MRB_TT_DATA);

  mrb_define_method(mrb, font_class, "initialize", font_initialize,
                    MRB_ARGS_REQ(2));
  mrb_define_method(mrb, font_class, "size", font_size, MRB_ARGS_NONE());
  mrb_define_method(mrb, font_class, "calc_bounds", font_calc_bounds,
                    MRB_ARGS_ARG(1, 1));
  mrb_define_method(mrb, font_class, "ascender", font_ascender_get,
                    MRB_ARGS_NONE());

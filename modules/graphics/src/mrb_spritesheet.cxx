...
spritesheet_generate_buffers(mrb_state* mrb, mrb_value self)
{
  Moon::Texture* texture =
    mmrb_valid_texture_ptr(mrb, moon_iv_get(mrb, self, KEY_TEXTURE));
  Moon::VertexBuffer* vbo =
    mmrb_vertex_buffer_ptr(mrb, moon_iv_get(mrb, self, KEY_VBO));
...
spritesheet_copy_quad_to(mrb_state* mrb, mrb_value self)
{
  ...
  mrb_get_args(mrb, "dfffi|H", &dest_vbo, &vbo_data_type, &x, &y, &z, &offset,
               &options);
...
mmrb_spritesheet_init(mrb_state* mrb)
{
  ...
  struct RClass* ss_cls =
    mrb_define_class_under(mrb, mod, "Spritesheet", mrb->object_class);

  mrb_define_method(mrb, ss_cls, "generate_buffers",
                    spritesheet_generate_buffers, MRB_ARGS_NONE());
  mrb_define_method(mrb, ss_cls, "copy_quad_to", spritesheet_copy_quad_to,
                    MRB_ARGS_ARG(5, 1));

 ...
 */
GLuint
Shader::CreateShader(const std::string contents, GLenum type)
{
...
GLuint
Shader::CreateProgram(const std::string vertexContent,
                      const std::string fragmentContent)
{
  ...
  glDeleteShader(
    shader); // http://stackoverflow.com/questions/9113154/proper-way-to-delete-glsl-shader
...
GLint
Shader::InitAttribute(std::string name)
{
  GLint attribute = glGetAttribLocation(m_program, name.c_str());
  if (attribute == -1) {
    fprintf(stderr, "Shader(program_id: %i): OpenGL Error 0x%04x: Could not "
                    "bind attribute %s\n",
...
GLint
Shader::InitUniform(std::string name)
{
...
GLint
Shader::Attribute(std::string name)
{
  assert(m_program);
...
GLint
Shader::Uniform(std::string name)
{
  assert(m_program);
...
void
Shader::SetUniform(std::string name, const GLint v1)
{
  glUniform1i(Uniform(name), v1);
}

void
Shader::SetUniform(std::string name, const GLfloat v1)
{
...
void
Shader::SetUniform(std::string name, const Moon::Vector2& vec)
{
  glUniform2fv(Uniform(name), 1, glm::value_ptr(vec));
}

void
Shader::SetUniform(std::string name, const Moon::Vector3& vec)
{
  glUniform3fv(Uniform(name), 1, glm::value_ptr(vec));
}

void
Shader::SetUniform(std::string name, const Moon::Vector4& vec)
{
  glUniform4fv(Uniform(name), 1, glm::value_ptr(vec));
}

void
Shader::SetUniform(std::string name, const Moon::Matrix4& mat)
{

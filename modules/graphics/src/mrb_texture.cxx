...
MOON_C_API const struct mrb_data_type texture_data_type = { "Texture",
                                                            texture_free };
...
mmrb_texture_init(mrb_state* mrb)
{
  ...
  struct RClass* texture_class =
    mrb_define_class_under(mrb, mod, "Texture", mrb->object_class);
  MRB_SET_INSTANCE_TT(texture_class, MRB_TT_DATA);

  mrb_define_method(mrb, texture_class, "initialize", texture_initialize,
                    MRB_ARGS_REQ(1));

...
MOON_C_API const struct mrb_data_type shader_data_type = { "Moon::Shader",
                                                           shader_free };

/* Initializes a new shader object, generating a vertex and fragment shader
 * pair.
 *
    ...
    case MRB_TT_DATA: {
      ...
      /*if ((ptr = mrb_data_check_get_ptr(mrb, data, &vector1_data_type))) {
        shader->SetUniform(name, *static_cast<Moon::Vector1*>(ptr));
                       ...
                       mrb_data_check_get_ptr(mrb, data, &vector2_data_type))) {
        shader->SetUniform(name, *static_cast<Moon::Vector2*>(ptr));

      } else if ((ptr =
                    mrb_data_check_get_ptr(mrb, data, &vector3_data_type))) {
        shader->SetUniform(name, *static_cast<Moon::Vector3*>(ptr));

      } else if ((ptr =
                    mrb_data_check_get_ptr(mrb, data, &vector4_data_type))) {
        shader->SetUniform(name, *static_cast<Moon::Vector4*>(ptr));

      } else if ((ptr =
                    mrb_data_check_get_ptr(mrb, data, &matrix4_data_type))) {
        shader->SetUniform(name, *static_cast<Moon::Matrix4*>(ptr));

      } else {
        mrb_raisef(mrb, E_TYPE_ERROR,
                   "unexpected object %S (expected Vector4 or Matrix4)",
                   mrb_obj_classname(mrb, data));
      }
    } break;
    default:
      mrb_raisef(mrb, E_TYPE_ERROR, "unexpected object %S",
                 mrb_obj_classname(mrb, data));
      break;
  }
  return self;
...
mmrb_shader_init(mrb_state* mrb)
{
  ...
  struct RClass* shader_class =
    mrb_define_class_under(mrb, mod, "Shader", mrb->object_class);
  MRB_SET_INSTANCE_TT(shader_class, MRB_TT_DATA);

  mrb_define_method(mrb, shader_class, "initialize", shader_initialize,
                    MRB_ARGS_REQ(2));
  mrb_define_method(mrb, shader_class, "use", shader_use, MRB_ARGS_NONE());
  mrb_define_method(mrb, shader_class, "set_uniform", shader_set_uniform,
                    MRB_ARGS_REQ(2));

  mrb_define_class_method(mrb, shader_class, "is_legacy",
                          shader_s_is_legacy_get, MRB_ARGS_NONE());
  mrb_define_class_method(mrb, shader_class, "is_legacy=",
                          shader_s_is_legacy_set, MRB_ARGS_REQ(1));
}

...
public:
  ...
  void Render(GLenum mode, const int offset); // adds offset to the indices used
                                              // (draws vertex at vbo[indice +
                                              // offset])
  void PushBack(Vertex v);
  void PushBackVertices(Vertex* v, int vertex_count);
  void PushBackIndices(GLuint i[], int index_count);
  void PushBack(
    Vertex* v, int vertex_count, GLuint i[],
    int index_count); // indices need to be relative to the vertices pushed in

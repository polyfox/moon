...
#define IVset(_name_, _value_)                                                 \
  mrb_iv_set(mrb, self, mrb_intern_lit(mrb, _name_), _value_)

#define KEY_SHADER "@shader"
#define KEY_VBO "@vbo"
#define KEY_ORIGIN "@origin"

#define MOON_GET_CLASS(__name__)                                               \
  mrb_class_get_under(mrb, mrb_module_get(mrb, "Moon"), __name__)
#define MOON_GET_MODULE(__name__)                                              \
  mrb_module_get_under(mrb, mrb_module_get(mrb, "Moon"), __name__)
;
...
moon_iv_get(mrb_state* mrb, mrb_value self, const char* name)
{
  ...
  if (mrb_nil_p(iv_value)) {
    mrb_raisef(mrb, E_ARGUMENT_ERROR, "Cannot use a nil %S",

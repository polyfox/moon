 ...
 */
static inline int
mmrb_vector_unroll(mrb_state* mrb, size_t argc, mrb_value* vals,
                   size_t result_size, mrb_float* result)
{
  if (argc < 1 || argc > result_size) {
    mrb_raisef(mrb, E_ARGUMENT_ERROR,
               "wrong number of arguments (%S for 1..%S)",
               mrb_fixnum_value(argc), mrb_fixnum_value(result_size));
  }
    ...
    switch (mrb_type(val)) {
      ...
      case MRB_TT_DATA: {
        ...
        if (t == &vector1_data_type) {
          ...
          mrb_raisef(mrb, E_TYPE_ERROR, "wrong type %S (expected a Vector*)",
                     mrb_str_new_cstr(mrb, mrb_obj_classname(mrb, val)));
        }
      }
      default: {
        mrb_raisef(mrb, E_TYPE_ERROR, "wrong type %S (expected Array, Numeric, "
  ...
  if (index == 1) {
    ...
    mrb_raisef(mrb, E_ARGUMENT_ERROR, "unrolled %S parameter(s) (required %S)",
               mrb_fixnum_value(index), mrb_fixnum_value(result_size));
  }
  return index;
}

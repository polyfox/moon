...
chars_to_utf8(const char* str)
{
  ...
  const size_t length = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str,
                                            sizeof(str), NULL, 0);
  wchar_t* name = new wchar_t[length];
  MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str, sizeof(str), name,
                      length);
  return name;
#else
...
public:
  ...
  ~WString() { delete[] m_str; }

  wchar_t operator[](size_t index) const { return m_str[index]; }
...
public:
  ...
  char* get_cptr(size_t index) const { return &(m_str[index]); }

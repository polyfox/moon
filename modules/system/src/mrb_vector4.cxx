...
#define m_vector_operator(__op__)                                              \
  return mmrb_vector4_value(mrb, get_vector4_value(mrb, self)                  \
                                   __op__ vector4_from_mrb_args(mrb));

#define m_vector_int_operator(__op__)                                          \
  return mmrb_vector4_value(                                                   \
    mrb, Moon::Vector4(glm::ivec4(get_vector4_value(mrb, self))                \
                         __op__ glm::ivec4(vector4_from_mrb_args(mrb))));
...
MOON_C_API const struct mrb_data_type vector4_data_type = { "Moon::Vector4",
                                                            vector4_free };

DEF_VEC_HELPERS(vector4, Moon::Vector4, mmrb_get_vector4_class(mrb),
                &vector4_data_type);
...
{
  cleanup_vector4(mrb, self);
  mrb_data_init(self, new Moon::Vector4(vector4_from_mrb_args(mrb)),
                &vector4_data_type);
  return self;
}
...
vector4_eq(mrb_state* mrb, mrb_value self)
{
  ...
  if (mrb_obj_is_kind_of(mrb, other, mmrb_get_vector4_class(mrb))) {
    return mrb_bool_value((*mmrb_vector4_ptr(mrb, self)) ==
                          (*mmrb_vector4_ptr(mrb, other)));
  }
...
{
  return mmrb_vector4_value(
    mrb, Moon::Vector4(~(glm::ivec4(*mmrb_vector4_ptr(mrb, self)))));
}
...
vector4_dot(mrb_state* mrb, mrb_value self)
{
  return mrb_float_value(
    mrb, glm::dot(get_vector4_value(mrb, self), vector4_from_mrb_args(mrb)));
}
...
vector4_distance(mrb_state* mrb, mrb_value self)
{
  ...
  return mrb_float_value(mrb,
                         glm::distance(get_vector4_value(mrb, self), *other));
}
...
mmrb_vector4_init(mrb_state* mrb)
{
  ...
  struct RClass* vector4_class =
    mrb_define_class_under(mrb, mod, "Vector4", mrb->object_class);
  MRB_SET_INSTANCE_TT(vector4_class, MRB_TT_DATA);

  mrb_define_method(mrb, vector4_class, "initialize", vector4_initialize,
                    MRB_ARGS_OPT(4));
  mrb_define_method(mrb, vector4_class, "initialize_copy",
                    vector4_initialize_copy, MRB_ARGS_REQ(1));
  /* coercion */
  mrb_define_method(mrb, vector4_class, "coerce", vector4_coerce,
                    MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, vector4_class, "+@", vector4_identity,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector4_class, "normalize", vector4_normalize,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector4_class, "length", vector4_length,
                    MRB_ARGS_NONE());
  ...
  // mrb_define_method(mrb, vector4_class, "cross",           vector4_cross,
  // MRB_ARGS_REQ(1));
  mrb_define_method(mrb, vector4_class, "distance", vector4_distance,
                    MRB_ARGS_REQ(1));
  /* bitwise operators */
  ...
  /* cast */
  mrb_define_class_method(mrb, vector4_class, "[]", vector4_s_cast,
                          MRB_ARGS_ANY());
  mrb_define_class_method(mrb, vector4_class, "extract", vector4_s_extract,
                          MRB_ARGS_REQ(1));

  mrb_alias_method(mrb, vector4_class, mrb_intern_cstr(mrb, "r"),
                   mrb_intern_cstr(mrb, "x"));
  mrb_alias_method(mrb, vector4_class, mrb_intern_cstr(mrb, "g"),
                   mrb_intern_cstr(mrb, "y"));
  mrb_alias_method(mrb, vector4_class, mrb_intern_cstr(mrb, "b"),
                   mrb_intern_cstr(mrb, "z"));
  mrb_alias_method(mrb, vector4_class, mrb_intern_cstr(mrb, "a"),

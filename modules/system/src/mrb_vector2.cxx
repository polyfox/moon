...
#define m_vector_operator(__op__)                                              \
  return mmrb_vector2_value(mrb, get_vector2_value(mrb, self)                  \
                                   __op__ vector2_from_mrb_args(mrb));

#define m_vector_int_operator(__op__)                                          \
  return mmrb_vector2_value(                                                   \
    mrb, Moon::Vector2(glm::ivec2(get_vector2_value(mrb, self))                \
                         __op__ glm::ivec2(vector2_from_mrb_args(mrb))));
...
MOON_C_API const struct mrb_data_type vector2_data_type = { "Moon::Vector2",
                                                            vector2_free };

DEF_VEC_HELPERS(vector2, Moon::Vector2, mmrb_get_vector2_class(mrb),
                &vector2_data_type);
...
{
  cleanup_vector2(mrb, self);
  mrb_data_init(self, new Moon::Vector2(vector2_from_mrb_args(mrb)),
                &vector2_data_type);
  return self;
}
...
vector2_eq(mrb_state* mrb, mrb_value self)
{
  ...
  if (mrb_obj_is_kind_of(mrb, other, mmrb_get_vector2_class(mrb))) {
    return mrb_bool_value((*mmrb_vector2_ptr(mrb, self)) ==
                          (*mmrb_vector2_ptr(mrb, other)));
  }
...
{
  return mmrb_vector2_value(
    mrb, Moon::Vector2(~(glm::ivec2(*mmrb_vector2_ptr(mrb, self)))));
}
...
vector2_dot(mrb_state* mrb, mrb_value self)
{
  return mrb_float_value(
    mrb, glm::dot(get_vector2_value(mrb, self), vector2_from_mrb_args(mrb)));
}
...
vector2_distance(mrb_state* mrb, mrb_value self)
{
  ...
  return mrb_float_value(mrb,
                         glm::distance(get_vector2_value(mrb, self), *other));
}
...
vector2_rotate(mrb_state* mrb, mrb_value self)
{
  ...
  return mmrb_vector2_value(
    mrb, glm::rotate(get_vector2_value(mrb, self), (float)angle));
}
...
vector2_lerp(mrb_state* mrb, mrb_value self)
{
  ...
  return mmrb_vector2_value(
    mrb, glm::lerp(get_vector2_value(mrb, self), *other, (float)delta));
}
...
mmrb_vector2_init(mrb_state* mrb)
{
  ...
  struct RClass* vector2_class =
    mrb_define_class_under(mrb, mod, "Vector2", mrb->object_class);
  MRB_SET_INSTANCE_TT(vector2_class, MRB_TT_DATA);

  mrb_define_method(mrb, vector2_class, "initialize", vector2_initialize,
                    MRB_ARGS_OPT(2));
  mrb_define_method(mrb, vector2_class, "initialize_copy",
                    vector2_initialize_copy, MRB_ARGS_REQ(1));
  /* coercion */
  mrb_define_method(mrb, vector2_class, "coerce", vector2_coerce,
                    MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, vector2_class, "+@", vector2_identity,
                    MRB_ARGS_NONE());
  ...
  /*mrb_define_method(mrb, vector2_class, "cross",           vector2_cross,
   * MRB_ARGS_REQ(1));*/
  /* */
  mrb_define_method(mrb, vector2_class, "normalize", vector2_normalize,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector2_class, "length", vector2_length,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector2_class, "distance", vector2_distance,
                    MRB_ARGS_REQ(1));
  mrb_define_method(mrb, vector2_class, "rotate", vector2_rotate,
                    MRB_ARGS_REQ(1));
  ...
  /* cast */
  mrb_define_class_method(mrb, vector2_class, "[]", vector2_s_cast,
                          MRB_ARGS_ANY());
  mrb_define_class_method(mrb, vector2_class, "extract", vector2_s_extract,
                          MRB_ARGS_REQ(1));
  /* gen */
  mrb_define_class_method(mrb, vector2_class, "circular_rand",
                          vector2_s_circular_rand, MRB_ARGS_REQ(1));
  mrb_define_class_method(mrb, vector2_class, "disk_rand", vector2_s_disk_rand,
                          MRB_ARGS_REQ(1));
}

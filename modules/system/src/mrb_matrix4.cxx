...
#define math_op_inline(__op__)                                                 \
  ...
  math_op_base(__op__, target_mat4) return self;
...
#define math_op(__op__)                                                        \
  ...
  math_op_base(__op__, target_mat4) return rtarget; /* */
...
MOON_C_API const struct mrb_data_type matrix4_data_type = { "Moon::Matrix4",
                                                            matrix4_free };
  ...
  if (argc == 1) {
    ...
    if (mrb_type(val) == MRB_TT_DATA) {
      if (DATA_TYPE(val) == &matrix4_data_type) { /* Matrix4 */
        ...
        source_mat4 =
          (Moon::Matrix4*)mrb_data_get_ptr(mrb, val, &matrix4_data_type);
        *mat = *source_mat4;
      } else {
    ...
    *mat = Moon::Matrix4(mrb_to_flo(mrb, args[0]), mrb_to_flo(mrb, args[1]),
                         mrb_to_flo(mrb, args[2]), mrb_to_flo(mrb, args[3]),

                         mrb_to_flo(mrb, args[4]), mrb_to_flo(mrb, args[5]),
                         mrb_to_flo(mrb, args[6]), mrb_to_flo(mrb, args[7]),

                         mrb_to_flo(mrb, args[8]), mrb_to_flo(mrb, args[9]),
                         mrb_to_flo(mrb, args[10]), mrb_to_flo(mrb, args[11]),

                         mrb_to_flo(mrb, args[12]), mrb_to_flo(mrb, args[13]),
                         mrb_to_flo(mrb, args[14]), mrb_to_flo(mrb, args[15]));
  } else {
    mrb_raisef(mrb, E_ARGUMENT_ERROR,
               "wrong argument count %d (expected 0, 1, 4, or 16)", argc);
  }
}
...
matrix4_eq(mrb_state* mrb, mrb_value self)
{
  ...
  if (mrb_obj_is_kind_of(mrb, other, mmrb_get_matrix4_class(mrb))) {
    return mrb_bool_value((*mmrb_matrix4_ptr(mrb, self)) ==
                          (*mmrb_matrix4_ptr(mrb, other)));
  }
...
matrix4_entry_get(mrb_state* mrb, mrb_value self)
{
  ...
  if (len == 1) {
    ...
    if (x < 0 || x >= 4) {
      mrb_raisef(mrb, E_INDEX_ERROR, "x %d is out of range (expected 0...4)",
                 x);
    ...
    if (y < 0 || y >= 4) {
      mrb_raisef(mrb, E_INDEX_ERROR, "y %d is out of range (expected 0...4)",
                 y);
      return mrb_nil_value();
    }
 ...
 * @return [Matrix4]
 */
static mrb_value matrix4_op_div(mrb_state* mrb, mrb_value self){ math_op(/) };
...
matrix4_translate_m(mrb_state* mrb, mrb_value self)
{
  ...
  if (len == 1) {
    ...
    target_mat4 = glm::translate(*mat4, glm::vec3(mrb_to_flo(mrb, vals[0]),
                                                  mrb_to_flo(mrb, vals[1]),
                                                  mrb_to_flo(mrb, vals[2])));
  } else {
    mrb_raisef(mrb, E_ARGUMENT_ERROR,
               "wrong argument count %d (expected 1 or 3)", len);
  }
  return target_mat4;
};
...
matrix4_rotate_m(mrb_state* mrb, mrb_value self)
{
  ...
  if (len == 2) {
    ...
    target_mat4 =
      glm::rotate(*mat4, glm::radians((float)mrb_to_flo(mrb, vals[0])),
                  glm::vec3(mrb_to_flo(mrb, vals[1]), mrb_to_flo(mrb, vals[2]),
                            mrb_to_flo(mrb, vals[3])));
  } else {
    mrb_raisef(mrb, E_ARGUMENT_ERROR,
               "wrong argument count %d (expected 2 or 4)", len);
  }
  return target_mat4;
}
...
matrix4_scale_m(mrb_state* mrb, mrb_value self)
{
  ...
  if (len == 1) {
    ...
    target_mat4 = glm::scale(*mat4, glm::vec3(mrb_to_flo(mrb, vals[0]),
                                              mrb_to_flo(mrb, vals[1]),
                                              mrb_to_flo(mrb, vals[2])));
  } else {
    mrb_raisef(mrb, E_ARGUMENT_ERROR,
...
matrix4_to_a16(mrb_state* mrb, mrb_value self)
{
  ...
  mrb_value argv[16] = {
    mrb_float_value(mrb, row1[0]), mrb_float_value(mrb, row1[1]),
    mrb_float_value(mrb, row1[2]), mrb_float_value(mrb, row1[3]),

    mrb_float_value(mrb, row2[0]), mrb_float_value(mrb, row2[1]),
    mrb_float_value(mrb, row2[2]), mrb_float_value(mrb, row2[3]),

    mrb_float_value(mrb, row3[0]), mrb_float_value(mrb, row3[1]),
    mrb_float_value(mrb, row3[2]), mrb_float_value(mrb, row3[3]),

    mrb_float_value(mrb, row4[0]), mrb_float_value(mrb, row4[1]),
    mrb_float_value(mrb, row4[2]), mrb_float_value(mrb, row4[3])
  };
...
mmrb_matrix4_init(mrb_state* mrb)
{
  ...
  struct RClass* matrix4_class =
    mrb_define_class_under(mrb, mod, "Matrix4", mrb->object_class);
  MRB_SET_INSTANCE_TT(matrix4_class, MRB_TT_DATA);

  mrb_define_method(mrb, matrix4_class, "initialize", matrix4_initialize,
                    MRB_ARGS_ANY());
  mrb_define_method(mrb, matrix4_class, "initialize_copy",
                    matrix4_initialize_copy, MRB_ARGS_REQ(1));

  mrb_define_method(mrb, matrix4_class, "coerce", matrix4_coerce,
                    MRB_ARGS_REQ(1));

  mrb_define_method(mrb, matrix4_class, "==", matrix4_eq, MRB_ARGS_REQ(1));

  mrb_define_method(mrb, matrix4_class, "[]", matrix4_entry_get,
                    MRB_ARGS_ARG(1, 1));
  mrb_define_method(mrb, matrix4_class, "[]=", matrix4_entry_set,
                    MRB_ARGS_ARG(2, 1));

  mrb_define_method(mrb, matrix4_class, "-@", matrix4_op_negate,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, matrix4_class, "+@", matrix4_op_identity,
                    MRB_ARGS_NONE());
  ...
  // mrb_define_method(mrb, matrix4_class, "%",               op_mod,
  // MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, matrix4_class, "clear", matrix4_clear,
                    MRB_ARGS_NONE());

  mrb_define_method(mrb, matrix4_class, "translate", matrix4_translate,
                    MRB_ARGS_ANY());
  mrb_define_method(mrb, matrix4_class, "rotate", matrix4_rotate,
                    MRB_ARGS_ANY());
  mrb_define_method(mrb, matrix4_class, "scale", matrix4_scale, MRB_ARGS_ANY());

  mrb_define_method(mrb, matrix4_class, "translate!", matrix4_translate_bang,
                    MRB_ARGS_ANY());
  mrb_define_method(mrb, matrix4_class, "rotate!", matrix4_rotate_bang,
                    MRB_ARGS_ANY());
  mrb_define_method(mrb, matrix4_class, "scale!", matrix4_scale_bang,
                    MRB_ARGS_ANY());

  mrb_define_method(mrb, matrix4_class, "to_a16", matrix4_to_a16,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, matrix4_class, "to_a", matrix4_to_a, MRB_ARGS_NONE());

  mrb_define_class_method(mrb, matrix4_class, "[]", matrix4_s_cast,
                          MRB_ARGS_ANY());
  mrb_define_class_method(mrb, matrix4_class, "ortho", matrix4_s_ortho,
                          MRB_ARGS_REQ(6));
  // mrb_define_class_method(mrb, matrix4_class, "extract",   matrix4_s_extract,
  // MRB_ARGS_REQ(1));
}

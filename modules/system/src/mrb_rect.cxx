...
MOON_C_API const struct mrb_data_type rect_data_type = { "Moon::IntRect",
                                                         rect_free };
...
mmrb_to_rect(mrb_state* mrb, mrb_value self)
{
  ...
  switch (type) {
    case MRB_TT_DATA: {
      Moon::IntRect* rect = static_cast<Moon::IntRect*>(
        mrb_data_check_get_ptr(mrb, self, &rect_data_type));
      if (rect)
        return *rect;
    }; break;
    case MRB_TT_ARRAY: {
      ...
      if (alen != 4) {
        mrb_raisef(mrb, E_ARGUMENT_ERROR, "wrong Array size %S (expected %S)",
                   mrb_fixnum_to_str(mrb, mrb_fixnum_value(alen), 10),
                   mrb_str_new_cstr(mrb, "4"));
      } else {
        return Moon::IntRect(mrb_int(mrb, mrb_ary_entry(self, 0)),
...
mmrb_to_float_rect(mrb_state* mrb, mrb_value obj)
{
  ...
  switch (type) {
    case MRB_TT_DATA: {
      Moon::IntRect* rect = static_cast<Moon::IntRect*>(
        mrb_data_check_get_ptr(mrb, obj, &rect_data_type));
      if (rect)
        return Moon::FloatRect(rect->x, rect->y, rect->w, rect->h);
    }; break;
    case MRB_TT_ARRAY: {
      ...
      if (alen != 4) {
        ...
        return Moon::FloatRect(mrb_to_flo(mrb, mrb_ary_entry(obj, 0)),
...
mmrb_rect_init(mrb_state* mrb)
{
  ...
  struct RClass* rect_class =
    mrb_define_class_under(mrb, mod, "Rect", mrb->object_class);
  MRB_SET_INSTANCE_TT(rect_class, MRB_TT_DATA);

  mrb_define_method(mrb, rect_class, "initialize", rect_initialize,
                    MRB_ARGS_ARG(0, 4));
  mrb_define_method(mrb, rect_class, "initialize_copy", rect_initialize_copy,
                    MRB_ARGS_REQ(1));

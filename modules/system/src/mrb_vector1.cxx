...
#define m_vector_operator(__op__)                                              \
  return mmrb_vector1_value(mrb, get_vector1_value(mrb, self)                  \
                                   __op__ vector1_from_mrb_args(mrb));

#define m_vector_int_operator(__op__)                                          \
  return mmrb_vector1_value(                                                   \
    mrb, Moon::Vector1(Moon::Vector1i(get_vector1_value(mrb, self))            \
                         __op__ Moon::Vector1i(vector1_from_mrb_args(mrb))));
...
MOON_C_API const struct mrb_data_type vector1_data_type = { "Moon::Vector1",
                                                            vector1_free };

DEF_VEC_HELPERS(vector1, Moon::Vector1, mmrb_get_vector1_class(mrb),
                &vector1_data_type);
...
mmrb_vector1_extract_mrb_array(mrb_state* mrb, mrb_value obj)
{
  ...
  if (_ary_len != 1) {
    mrb_raisef(mrb, E_ARGUMENT_ERROR, "wrong array size %d (expected 1)",
               _ary_len);
  } else {
...
mmrb_vector1_extract_args(mrb_state* mrb, int argc, mrb_value* vals)
{
  ...
  switch (argc) {
    case 1:
      ...
      break;
    default:
      mrb_raisef(mrb, E_ARGUMENT_ERROR, "wrong number of arguments (%d for 1)",
                 argc);
  }
  return result;
}
...
{
  cleanup_vector1(mrb, self);
  mrb_data_init(self, new Moon::Vector1(vector1_from_mrb_args(mrb)),
                &vector1_data_type);
  return self;
}
...
vector1_eq(mrb_state* mrb, mrb_value self)
{
  ...
  if (mrb_obj_is_kind_of(mrb, other, mmrb_get_vector1_class(mrb))) {
    return mrb_bool_value((*mmrb_vector1_ptr(mrb, self)) ==
                          (*mmrb_vector1_ptr(mrb, other)));
  }
...
{
  return mmrb_vector1_value(
    mrb, Moon::Vector1(~(Moon::Vector1i(*mmrb_vector1_ptr(mrb, self)))));
}
...
vector1_dot(mrb_state* mrb, mrb_value self)
{
  return mrb_float_value(
    mrb, glm::dot(get_vector1_value(mrb, self), vector1_from_mrb_args(mrb)));
}
...
vector1_distance(mrb_state* mrb, mrb_value self)
{
  ...
  return mrb_float_value(mrb,
                         glm::distance(get_vector1_value(mrb, self), *other));
}
...
vector1_lerp(mrb_state* mrb, mrb_value self)
{
  ...
  return mmrb_vector1_value(
    mrb, Moon::Vector1(
           glm::lerp(mmrb_vector1_ptr(mrb, self)->x, other->x, (float)delta)));
}
...
mmrb_vector1_init(mrb_state* mrb)
{
  ...
  struct RClass* vector1_class =
    mrb_define_class_under(mrb, mod, "Vector1", mrb->object_class);
  MRB_SET_INSTANCE_TT(vector1_class, MRB_TT_DATA);

  mrb_define_method(mrb, vector1_class, "initialize", vector1_initialize,
                    MRB_ARGS_OPT(1));
  mrb_define_method(mrb, vector1_class, "initialize_copy",
                    vector1_initialize_copy, MRB_ARGS_REQ(1));
  /* coercion */
  mrb_define_method(mrb, vector1_class, "coerce", vector1_coerce,
                    MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, vector1_class, "x=", vector1_x_setter,
                    MRB_ARGS_REQ(1));
  ...
  mrb_define_method(mrb, vector1_class, "+@", vector1_identity,
                    MRB_ARGS_NONE());
  ...
  /* */
  mrb_define_method(mrb, vector1_class, "normalize", vector1_normalize,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector1_class, "length", vector1_length,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector1_class, "distance", vector1_distance,
                    MRB_ARGS_REQ(1));
  ...
  /* conversion */
  mrb_define_method(mrb, vector1_class, "to_int", vector1_to_int,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, vector1_class, "to_i", vector1_to_int,
                    MRB_ARGS_NONE());
  ...
  /* cast */
  mrb_define_class_method(mrb, vector1_class, "[]", vector1_s_cast,
                          MRB_ARGS_ANY());
  mrb_define_class_method(mrb, vector1_class, "extract", vector1_s_extract,
                          MRB_ARGS_REQ(1));
}
